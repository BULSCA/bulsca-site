...
@section('content')
    <x-page-banner
        title="{{ $season->name }}"
        subtitle="{{ $season->from->format('Y') }}-{{ $season->to->format('y') }}"
        :snowContainer="true"
    />
    ...
    <div class=" container-responsive ">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 md:gap-y-9">
            @foreach ($comps as $comp)
                @php
                    $dateDiff = now()->startOfDay()->diffInDays($comp->when->startOfDay(), false); // Calculate the difference in days
        ...
        @foreach ($data['data'] as $row)
            ...
            <tr>
                ...
                @foreach ($data['comps'] as $comp)
                  <td>
                      @if (!empty($row['positionPoints'][$comp]) && $row['positionPoints'][$comp] != 0)
                          {{ max(11 - $row['positionPoints'][$comp], 1) }} (@th($row['positionPoints'][$comp]))
                      @else
                          –
                      @endif
